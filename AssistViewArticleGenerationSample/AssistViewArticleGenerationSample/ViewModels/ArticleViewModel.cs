...
namespace AssistViewArticleGenerationSample.ViewModels
{
    /// <summary>
    ...
    public class ArticleViewModel : BaseViewModel
    {
        ...
        public string MarkdownContent
        {
            get
            {
                try
                {
                    // Normalize simple HTML line breaks before conversion
                    var html = _htmlContent ?? string.Empty;
                    ...
                    // Ensure spacing between paragraphs for better readability
                    md = md.Replace("\n\n\n", "\n\n");

                    // Make the first non-empty line bold if it's not already a markdown heading/bold
                    var lines = md.Split('\n');
                    for (int i = 0; i < lines.Length; i++)
                    {
                        ...
                        {
                            bool isHeading = trimmed.StartsWith("#");
                            bool isAlreadyBold = trimmed.StartsWith("**") && trimmed.EndsWith("**") && trimmed.Length > 4;
                            if (!isHeading && !isAlreadyBold)
                            {
                                lines[i] = $"**{trimmed}**";
                            }
                            break;
